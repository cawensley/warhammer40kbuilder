export default function army(state = { Name: '', SquadArray: [] }, action) {
  if (action.type === 'Army_Name_Change') {
    return { ...state, Name: action.payload };
  }
  if (action.type === 'Army_Squad_Change') {
    return { ...state, SquadArray: action.payload };
  }
  if (action.type === 'Army_Squad_Row_Add') {
    const squadRow = {
  ...
  if (action.type === 'Army_Squad_Row_Rem') {
    const newSquads = state.SquadArray;
  ...
  if (action.type === 'Army_Unit_Row_Rem') {
    const newSquads = state.SquadArray;
  ...
  if (action.type === 'Army_Unit_Choice_Change') {
    const newSquads = state.SquadArray;
    newSquads[action.payload.roleIndex].Squads[action.payload.rowIndex]
      .Unit = action.payload.UnitID;
    return { ...state, SquadArray: newSquads };
  }
  if (action.type === 'Army_Unit_QTY_Change') {
    const newSquads = state.SquadArray;
    newSquads[action.payload.roleIndex].Squads[action.payload.rowIndex]
      .UnitQTY = action.payload.UnitQTY;
    return { ...state, SquadArray: newSquads };
  }
  if (action.type === 'Army_Equipment_Row_Add') {
    const equipmentRow = {
      Squad: null,
      UnitQTY: null,
      Unit: null,
    ...
    const newSquads = state.SquadArray;
    newSquads[action.payload.roleIndex].Squads.splice(action.payload.rowIndex + 1, 0, equipmentRow);
    return { ...state, SquadArray: newSquads };
  }
  if (action.type === 'Army_Equipment_Row_Rem') {
    const newSquads = state.SquadArray;
    newSquads[action.payload.roleIndex].Squads.splice(action.payload.rowIndex, 1);
    return { ...state, SquadArray: newSquads };
  }
  if (action.type === 'Army_Equipment_Choice_Change') {
    const newSquads = state.SquadArray;
    newSquads[action.payload.roleIndex].Squads[action.payload.rowIndex]
