...
function EditEquipmentPage ({match}) {
    ...
    function handleNameInput(input) {setState({...state,Equipment:{...state.Equipment,Name:input}})}
    function handleCostInput(input) {setState({...state,Equipment:{...state.Equipment,Cost:+input}})}

    function getEditItemInfo () {
        setState({...state,isLoading:true});
        firebase.firestore().collection("equipment").doc(editEquipmentID).get()
            .then(doc=>{
                setState({...state,Equipment:{...state.Equipment,Name:doc.data().Name,Cost:doc.data().Cost}});
            });
